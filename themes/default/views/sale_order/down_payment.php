...
$( document ).ready(function() {	
	var site = <?= json_encode(array('base_url' => base_url(), 'settings' => $Settings, 'dateFormats' => $dateFormats)) ?> 
		...
		$("#priciple_loan").on("change",function()
		{
			...
			$(this).val((total-(order_discount+deposit+down_pay)).toFixed(2));
		...
		$(document).on('change','#depreciation_type_1, #depreciation_rate_1, #depreciation_term_1, #loan_amount, #frequency, #down_date',function() {
		...
		$(document).on('change','#principle_type_1, #loan_amount,#down_date',function()
		{
			var option = $("#principle_type_1 option:selected").val();
			if(option!='none')
			{
	...
	function depreciation(amount,rate,term_of_day,frequency,p_type,total_amount, start_date){
		...
		var term = (term_of_day/frequency).toFixed(0);
		frequency = parseFloat(frequency);
		var priciple_term = $("#priciple_term").val();
			...
			if(rate == '' || rate < 0) {
				...
				if(term == '' || term <= 0) {
					...
					for(i=1;i<=priciple_term;i++){
						if(i== 1){
							var dateline  = moment(start_date).format('DD/MM/YYYY');
								principle = priciple_loan/priciple_term;
								balance  -= principle;
							if(balance   <= 0){
								balance   = 0;
							}
					...
					if(p_type == 1){
						...
						var percent = 0;
						var rate_amount = ((rate/100));
						var payment = ((balance * rate_amount)*((Math.pow((1+rate_amount),term))/(Math.pow((1+rate_amount),term)-1)));
