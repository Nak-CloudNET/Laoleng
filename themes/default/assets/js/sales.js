...
function loadItems() {
	if (localStorage.getItem('slitems')) {
		...
		var tran_type = localStorage.getItem('transaction_type');
		$("#slTable tbody").empty();
		...
		$.each(slitems, function () {
			...
			var product_id 		= item.row.id,
				...
				console.log(item.row.real_unit_price);
				var exchange_rate = $("#exchange_rate").val();
			...
			if(tran_type){
				tr_html += '<td><input class="form-control text-center rquantity" name="quantity[]" type="text" value="' + formatDecimal(item_qty) + '" data-id="' + row_no + '" data-item="' + item_id + '" id="quantity_' + row_no + '" onClick="this.select();" readonly></td>';
			}else{
