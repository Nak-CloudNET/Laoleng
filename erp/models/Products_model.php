...
class Products_model extends CI_Model
{
	...
	public function getUsingStockProducts($term, $warehouse_id, $plan = NULL, $limit = 100)
    {
		...
		if ($plan) {
			$this->db->where("project_plan.id", $plan);
		}
		$this->db->where("warehouses_products.warehouse_id", $warehouse_id);
        $this->db->limit($limit);
		$this->db->select('products.*, warehouses_products.quantity as qoh, units.name as unit_name, (COALESCE(erp_project_plan_items.quantity_balance, 0) - COALESCE(erp_project_plan_items.quantity_used, 0)) as project_qty');
		$this->db->join('warehouses_products', 'warehouses_products.product_id = products.id', 'left');
		$this->db->join('units', 'units.id = products.unit', 'left');
		$this->db->join('project_plan_items', 'products.id = project_plan_items.product_id', 'left');
		$this->db->join('project_plan', 'project_plan.id = project_plan_items.project_plan_id', 'left');
        $this->db->group_by('products.id');
        $q = $this->db->get('products');
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
	...
	public function addMultiAdjustment($data, $dataPurchase = NULL)
    {
        if ($this->db->insert('adjustments', $data)) {
			...
			foreach($dataPurchase as $products){
				...
				$this->db->update("inventory_valuation_details",array('cost'=>$product_cost->cost,'avg_cost'=>$product_cost->cost,'date'=>$data['date'],'reference_no'=>$data['reference_no']),array('field_id'=>$adjust_item_id));
			}
	...
	{
		$this->db->select('enter_using_stock.*,warehouses.name,users.first_name,users.last_name, project_plan.plan, products.cf3, products.cf4');
