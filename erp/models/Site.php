...
class Site extends CI_Model
{
	...
	*********/
	...
	public function calculateAVGCost2017($product_id, $shipping, $quantity, $price, $total_price, $cost, $item_discount, $order_discount, $variant_id, $subtotal, $total)
	{
		$oldcost   				= $this->getoldcost($product_id);
		$old_cost  				= $oldcost->cost;
		$old_qty   				= $oldcost->quantity;
		...
		*/
		//Total New Cost = (Subtotl * (Total + Shipping))/Total
...
//						echo 'hello'; exit();
        ...
        foreach ($variants as $variant) {
            ...
            if ($this->db->update('product_variants', array('quantity' => $pvar_qty), array('id' => $variant->id))) {
                if ($this->getWarehouseProductsVariants($variant->id, $warehouse_id)) {
                    $this->db->update('warehouses_products_variants', array('quantity' => ($wvar_qty)), array('option_id' => $variant->id, 'warehouse_id' => $warehouse_id, 'product_id' => $product_id));
                } else {
                    if ($wvar_qty) {
	...
	public function getAllBillerData() {
        $this->db->where('group_name','biller');
        $q = $this->db->get('companies');
        if ($q->num_rows() > 0) {
            return $q->row();
