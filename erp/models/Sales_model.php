...
class Sales_model extends CI_Model
{
    ...
    public function getAllInvoiceItems($sale_id)
    {
        $this->db->select("sale_items.*,
							...
							sales.total,sales.grand_total,
							sales.shipping,sales.order_tax
        				")
  ...
  //           ->group_by('deliveries.id')
	...
	{
		$this->db->select('purchases.id, purchases.date,purchases.reference_no,purchases.paid,purchases.biller_id,purchases.supplier_id,payments.paid_by')
...
/* 	public function addPaymentLoan($data = array(),$id=NULL)
    {
		$period_id = $data['period'];
		$sale_loan = $this->sales_model->getSaleId($period_id);

        if ($this->db->update('loans', $data, array('period' => $period_id,'sale_id' => $id))) {
			return true;
        }
        return false;
    } */
	...
	public function addPaymentLoan($data = array())
    {
		...
		$old_loan  = $this->sales_model->getSingleLoanById($id);
	...
	public function getSaleItemsBySaleId($id){
        $this->db->select('erp_sale_items.*, erp_products.name AS pro_name,erp_units.name AS unit_name,erp_product_variants.name AS variant_name, erp_product_variants.qty_unit,, erp_sales.order_discount, erp_sales.order_tax, erp_sales.shipping')
                ->from('erp_sale_items')
	...
	public function Addloans($loans=null,$sale_id=null,$loan_info=array(),$update=null)
	{
	...
	/*public function getExpiryDateByID($product_code, $warehouse_id){
		$this->db->select('erp_purchase_items.expiry')
			->from('erp_purchase_items')
			->where(array('erp_purchase_items.product_code' => $product_code, 'erp_purchase_items.warehouse_id' => $warehouse_id, 'erp_purchase_items.quantity >' => 0));
		$q = $this->db->get();
		if($q->num_rows()>0){
			foreach($q->result() as $row){
				$data[] = $row;
	...
	public function getDownPayment($id) {
		...
		$q = $this->db->get_where('payments', array('sale_id' => $id, 'paid_by' => 'down'));
		if($q->num_rows() > 0) {
			return $q->row()->down;
		}
		return false;
	...
	{
		$this->db->select('term,total_interest,frequency,depreciation_type,principle_type,join_lease_id,term_id,down_amount,down_date');
		$q = $this->db->get_where('erp_sale_order', array('erp_sale_order.id' => $so_id));
	...
	public function getInvoiceBydigital_id($id){
		$this->db->select("products.name,SUM(erp_sale_items.quantity) as quantity,SUM(unit_price) as price");
		$this->db->join("products","products.id=erp_sale_items.digital_id","LEFT");
		$this->db->where("sale_id",$id);
		$this->db->group_by("digital_id");
		$q = $this->db->get('erp_sale_items');
        if ($q->num_rows() > 0) {
